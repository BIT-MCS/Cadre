...
class DANet(nn.Module):
    ...
    def get_latent_feature(self, x, mode):
        # [b, 512, 5, 8]
        layer4_x = self.backbone(x)
        # da_visual_x, da_bc_x = self.da_head(layer4_x)
        da_att_x = self.da_head(layer4_x)
        ...
        if self.pred_bc:
            ...
            if mode == "add":
                ...
                ppo_latent_feature = torch.cat((att_visual_x, att_bc_x), dim=-1)
        else:
            att_visual_x = self.visual_fc(da_visual_x)
    ...
    def get_bc_actions(self, x, bc_speed):
        # [b, 512, 5, 8]
        layer4_x = self.backbone(x)
        # da_visual_x, da_bc_x = self.da_head(layer4_x)
        da_att_x = self.da_head(layer4_x)
        ...
        return _pred_steer, _pred_throttle
    ...
    def get_light_state(self, x):
        ...
        _, lightState_pred_indices = torch.max(_reverse_lightState.data, 1)
        # total_losses['lightState_accuracy'] += (lightState_pred_indices == light_state).detach().cpu().sum().item()

        # [batch_size]
        # 0~3, 4 kinds of light state
        cur_light_state = lightState_pred_indices[0]

        return cur_light_state

def get_model(net_params):
    return DANet(net_params)