...
class AutonomousAgent(object):
    ...
    def __init__(self, args, traffic_light, counter, shared_model, shared_grad_buffers, performance_counter):
        ...
        self.setup(args, traffic_light, counter, shared_model, shared_grad_buffers, performance_counter)
    ...
    def reset_sensor_interface(self):
    ...
    def run_step(self, input_data, timestamp):
        ...
        reload = False
        error_message = ""
        return control, reload, error_message

    def update_completion(self, completion):
        self.completion_ratio = completion
    ...
    def __call__(self, lp_wrapper=None):
        """
        ...
        if lp_wrapper:
            control, reload, error_message = lp_wrapper(input_data, timestamp)
        else:
            control, reload, error_message = self.run_step(input_data, timestamp)
    ...
    def get_event(self):
        ...
        find out new event
        """
        # print('len criteria:', len(self.scenario.get_criteria()))
        node_cnt = 0
        new_event = []
        for node in self.scenario.get_criteria():
            if node.list_traffic_events:
                ...
                # print('cnt', node_cnt, 'len:', len(node.list_traffic_events))
                event_cnt = 0
