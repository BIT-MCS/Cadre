        ...
        while self._run_ps:
            if self._callback is not None:
                ...
                if current_time - latest_time > (1 / self._reading_frequency) \
                    ...
                    # print('current_time: ', current_time, latest_time, 1 / self._reading_frequency)
                    time.sleep(0.001)
...
class CallBack(object):
    ...
    def _parse_obstacle_cb(self, obstacle_data, tag):
        if ('static' in obstacle_data.other_actor.type_id or 'traffic' in obstacle_data.other_actor.type_id) \
                and 'sidewalk' not in obstacle_data.other_actor.type_id:
            return
        array = [obstacle_data.distance, obstacle_data.other_actor]
        self._data_provider.update_sensor(tag, array, obstacle_data.frame)

    def _parse_imu_cb(self, imu_data, tag):
        array = np.array([imu_data.transform.location.x,
                          imu_data.transform.location.y,
                          imu_data.transform.location.z,
                          imu_data.transform.rotation.yaw,
...
class SensorInterface(object):
    ...
    def register_sensor(self, tag, sensor_type, sensor):
        ...
        if sensor_type == "sensor.other.obstacle":
            self._obstacle_object[tag] = sensor
    ...
    def update_sensor(self, tag, data, timestamp):
        ...
        if tag in self._obstacle_object:
    ...
    def get_data(self):
        ...
        for key in self._obstacle_object.keys():
    ...
    # todo: add destroy sensor function
    def destroy(self):
        self._sensors_objects = {}
        self._obstacle_object = {}
        self._data_buffers = {}
    ...
    def clear_obstacle(self, tag):
        if tag in self._obstacle_object.keys():
            self._timestamps[tag] = -1
            self._data_buffers[tag] = None
...
#             if self._data_buffers[key] is None and key is not "obstacle":
#                 return False
#         return True
#
#     def get_data(self):
...
#
#     def clear_obstacle(self, tag):
#         if tag in self._sensors_objects.keys():
#             self._timestamps[tag] = -1
#             self._data_buffers[tag] = None