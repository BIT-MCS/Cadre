...
def interpolate_trajectory(st, ed, world, waypoints_trajectory, hop_resolution=1.0):
    """
    ...
    if st is not None:
        ...
        st_waypoint = waypoints_trajectory[0]
    # st_waypoint = waypoints_trajectory[st]
    ed_waypoint = waypoints_trajectory[-1]
    interpolated_trace = grp.trace_route(st_waypoint, ed_waypoint)
    for wp_tuple in interpolated_trace:
        route.append((wp_tuple[0].transform, wp_tuple[1]))
    # for i in range(st, ed):  # Goes until the one before the last.
    #     if i < 0 or i + 1 >= len(waypoints_trajectory):
    #         continue
    #
