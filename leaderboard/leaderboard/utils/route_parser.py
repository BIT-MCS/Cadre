...
class RouteParser(object):
    ...
    def parse_routes_file(route_filename, scenario_file, single_route=None):
        ...
        for route in tree.iter("route"):
            ...
            # if "all_towns_traffic_scenarios_public.json" not in scenario_file:
            if ".json" not in scenario_file:
                _scenario_file = scenario_file + '/' + ('route%02d' % int(route_id)) + '.json'
                # _scenario_file = scenario_file + '/' + ('route%03d' % int(route_id)) + '.json'
            else:
                _scenario_file = scenario_file
            new_config.scenario_file = _scenario_file
    ...
    def scan_route_for_scenarios(route_name, trajectory, world_annotations):
        ...
        for town_name in world_annotations.keys():
            ...
            for scenario in scenarios:  # For each existent scenario
                ...
                origin_scenario = {"scenario_type": scenario_name, "available_event_configurations": []}
                ...
                for event in scenario["available_event_configurations"]:
                    ...
                    if match_position is not None:
                        ...
                        origin_scenario["available_event_configurations"].append(event)
                origin_possible_scenarios.append(origin_scenario)
        return possible_scenarios, existent_triggers, origin_possible_scenarios
