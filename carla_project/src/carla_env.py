...
def get_nearby_lights(vehicle, lights, pixels_per_meter=5.5, size=512, radius=5):
    # print('in get_nearby_lights size', size)
    result = list()
...
# todo: remove draw and add return
def draw_traffic_lights(image, vehicle, lights, pixels_per_meter=5.5, size=512, radius=5):
    ...
    for light in lights:
        ...
        if dist > a + b:
            continue
        x, y = target
        # todo: only draw red light
        if light.state.real == 0:
            light_state = light.state.real
            draw.ellipse(
                (x - radius, y - radius, x + radius, y + radius),
                13 + light.state.real)
            light_dist = dist
            # print('x:', trigger.extent.x, ' y:', trigger.extent.y, ' z:', trigger.extent.z)
            # print('vehicle_x:', vehicle.bounding_box.extent.x, ' vehicle_y:', vehicle.bounding_box.extent.y, ' vehicle_z:',
...
class PedestrianPool(object):
    def __init__(self, client, n_pedestrians, ego_location):
        self.client = client
        self.world = client.get_world()
        ...
        for _ in range(n_pedestrians):
            points = self._get_spawn_point()
            if points:
                spawn_points.append(points)

        # print('spawn point:', len(spawn_points), spawn_points)
        ...
        # print('len(walkers)', len(walkers), walkers)
    ...
    def _get_actors(self):
        ...
        for _ in range(n_try):
            ...
            if spawn.location and spawn.location.distance(self.ego_location) < 100:
    ...
    def _get_spawn_point(self, n_retry=10):
        for _ in range(n_retry):
            ...
            try:
                ...
                #     if spawn.location.distance(self.ego_location) < 100:
                #         break
            if spawn.location is not None:
                return spawn
