...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    _actor_target_velocity_map = dict()
    ...
    #             print(actor.id, actor.is_alive)

    # @staticmethod
    # def recover_world():
    #     for actor in CarlaDataProvider._actor_target_velocity_map:
    #         if actor is not None and actor.is_alive:
    #             if CarlaDataProvider._actor_target_velocity_map[actor] is not None:
    #                 actor.set_target_velocity(CarlaDataProvider._actor_target_velocity_map[actor])
    #
    #     for actor in CarlaDataProvider._actor_location_map:
    #         if actor is not None and actor.is_alive:
    #             if CarlaDataProvider._actor_location_map[actor] is not None:
    #                 actor.set_location(CarlaDataProvider._actor_location_map[actor])
    #
    #     for actor in CarlaDataProvider._actor_transform_map:
    #         if actor is not None and actor.is_alive:
    #             if CarlaDataProvider._actor_transform_map[actor] is not None:
    #                 actor.set_transform(CarlaDataProvider._actor_transform_map[actor])
    #
    #     CarlaDataProvider._actor_target_velocity_map.clear()
    #
    # @staticmethod
    # def save_world_state():
    #     for actor in CarlaDataProvider._actor_velocity_map:
    #         if actor is not None and actor.is_alive:
    #             CarlaDataProvider._actor_target_velocity_map[actor] = actor.get_velocity()
    #
    #     for actor in CarlaDataProvider._actor_location_map:
    #         if actor is not None and actor.is_alive:
    ...
    def on_carla_tick():
        ...
        for actor in CarlaDataProvider._actor_velocity_map:
            if actor is not None and actor.is_alive:
                # if actor.id == 280:
                #     print('in on carla tick :', calculate_velocity(actor))
        ...
        if len(CarlaDataProvider._controllers) > 0:
            for i, controller in enumerate(CarlaDataProvider._controllers):
                CarlaDataProvider._timers[i] -= 1

                if CarlaDataProvider._timers[i] <= 0:
                    CarlaDataProvider._timers[i] = np.random.randint(60, 600) * 20
    ...
    def get_velocity(actor):
        ...
        for key in CarlaDataProvider._actor_velocity_map:
            if key.id == actor.id:
                # print('actor_id: ', actor.id, CarlaDataProvider._actor_velocity_map[key], calculate_velocity(actor))
    ...
    @staticmethod
    def get_test_velocity(actor):
        # print(actor.id, 'get_test_velocity: ', round(actor.get_velocity().x, 2), round(actor.get_velocity().y, 2))
        velocity_squared = actor.get_velocity().x ** 2
        velocity_squared += actor.get_velocity().y ** 2
    ...
    def reset_lights(reset_params):
        ...
        for param in reset_params:
            # print('in carla_data_provider:', param['state'], param['green_time'], param['red_time'],
            #       param['yellow_time'])
    ...
    def get_next_traffic_light_by_location(location):
        ...
        """
        waypoint = CarlaDataProvider.get_map().get_waypoint(location)
        # print(location, waypoint.transform.location)
        # Create list of all waypoints until next intersection
        ...
        relevant_traffic_light = None
        distance_to_relevant_traffic_light = 15
    ...
    @staticmethod
    def generate_random_spawn_points(num_of_points, n_retry=10):
        point_list = []
        for _ in range(num_of_points):
            spawn = carla.Transform()
            for _ in range(n_retry):
                spawn.location = CarlaDataProvider._world.get_random_location_from_navigation()
                # if spawn.location is not None:
                if spawn.location and spawn.location.distance(
                        CarlaDataProvider.get_hero_actor().get_transform().location) < 5:
                    break
            if spawn.location:
                point_list.append(spawn)
        return point_list

    # @staticmethod
    # def generate_spawn_points_around_hero(ego_vehicle):
    #     """
    #     Generate spawn points for the current map
    #     """
    #     ego_points = ego_vehicle.get_transform().location
    #     spawn_points = []
    #     for dx in range(-50, 50, 7):
    #         for dy in range(-50, 50, 7):
    #             _points = carla.Location(ego_points.x + dx, ego_points.y + dy, ego_points.x)
    #             _waypoint = CarlaDataProvider.get_map(CarlaDataProvider._world).get_waypoint(_points)
    #             if _waypoint is not None:
    #                 _waypoint = _waypoint.transform
    #                 if CarlaDataProvider.get_distance(_waypoint.location,
    #                                                   ego_points) < 50 and CarlaDataProvider.get_distance(
    #                     _waypoint.location, ego_points) > 7:
    #                     if _waypoint not in spawn_points:
    #                         spawn_points.append(_waypoint)
    #
    ...
    #     Generate spawn points for the current map
    #     """
    #     ego_points = CarlaDataProvider.get_hero_actor().get_transform().location
    #     spawn_points = []
    #     for dx in range(-50, 50, 7):
    #         for dy in range(-50, 50, 7):
    #             _points = carla.Location(ego_points.x + dx, ego_points.y + dy, ego_points.x)
    ...
    #
    #     # spawn_points = list(CarlaDataProvider.get_map(CarlaDataProvider._world).get_spawn_points())
    #     all_spawn_points = list(CarlaDataProvider.get_map(CarlaDataProvider._world).get_spawn_points())
    #     # if CarlaDataProvider._training:
    #     #     ego_points = CarlaDataProvider.get_hero_actor().get_transform().location
    #     #     spawn_points = []
    #     #     for _points in all_spawn_points:
    #     #         if CarlaDataProvider.get_distance(_points.location, ego_points) < 100:
    #     #             spawn_points.append(_points)
    #     # else:
    ...
    #
    #     for _points in all_spawn_points:
    #         if CarlaDataProvider.get_distance(_points.location, ego_points) < 5:
    #             continue
    #         if CarlaDataProvider._training:
    #             _points.location.z = ego_points.z
    #             if CarlaDataProvider.get_distance(_points.location, ego_points) < 30:
    #                 spawn_points.append(_points)
    #         else:
    #             spawn_points.append(_points)
    #     print('totally ', len(spawn_points), ' spawn points!')
    #     CarlaDataProvider._rng.shuffle(spawn_points)
    #     CarlaDataProvider._spawn_points = spawn_points
    #     CarlaDataProvider._spawn_index = 0
    ...
    def create_blueprint(model, rolename='scenario', color=None, actor_category="car"):
        ...
        # Set the model
        try:
            blueprint = CarlaDataProvider._blueprint_library.filter(model)
            if "vehicle" in model:
                blueprint = [x for x in blueprint if int(x.get_attribute('number_of_wheels')) == 4]
            blueprint = CarlaDataProvider._rng.choice(blueprint)
    ...
    def request_new_actor(model, spawn_point, rolename='scenario', autopilot=False,
        ...
        if actor is None:
            print("Error: Unable to spawn vehicle {} at {}".format(blueprint.id, spawn_point))
            return None
            # raise RuntimeError(
    ...
    # @staticmethod
    # def request_new_batch_actors_around_hero(ego_vehicle, model, amount, spawn_points, autopilot=False,
    ...
    #     # if "walker" in model:
    #     #
    #     #     CarlaDataProvider._pedestrian_pool = PedestrianPool(CarlaDataProvider._client, amount,
    #     #                                                         CarlaDataProvider.get_hero_actor().get_transform().location)
    #     #     return CarlaDataProvider.handle_actor_batch(CarlaDataProvider._pedestrian_pool.actors)
    #
    #     SpawnActor = carla.command.SpawnActor  # pylint: disable=invalid-name
    #     SetAutopilot = carla.command.SetAutopilot  # pylint: disable=invalid-name
    #     FutureActor = carla.command.FutureActor  # pylint: disable=invalid-name
    #
    #     # todo: debug
    #     # _spawn_points = CarlaDataProvider.generate_spawn_points_around_hero(ego_vehicle)
    #     _spawn_points = CarlaDataProvider.generate_spawn_points()
    #
    #
    #     batch = []
    #     _spawn_index = 0
    ...
    #
    #         if spawn_point:
    #             actor = SpawnActor(blueprint, spawn_point).then(
