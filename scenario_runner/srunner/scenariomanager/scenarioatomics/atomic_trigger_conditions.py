...
class StandStill(AtomicCondition):
    ...
    def update(self):
        ...
        if velocity > EPSILON and self.name != "followleadingvehicle":
            self._start_time = GameTime.get_time()
...
class WalkerCollision(AtomicCondition):
    def __init__(self, ego_vehicle, actor, name, duration=float("inf")):
        """
        Setup actor
        """
        super(WalkerCollision, self).__init__(name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._actor = actor
    ...
    def initialise(self):
        ...
        Initialize the start time of this condition
        """
        super(WalkerCollision, self).initialise()
    ...
    def update(self):
        ...
        """
        new_status = py_trees.common.Status.RUNNING
        scen_location = CarlaDataProvider.get_location(self._actor)
        # scen_location.z = 0
        if self._has_checked:
            return new_status
        for _, other_actors in CarlaDataProvider.get_actors():
            if other_actors.id != self._actor.id and other_actors.id != self.ego_vehicle_id and "vehicle" in other_actors.type_id:
                other_location = CarlaDataProvider.get_location(other_actors)
                if other_location:
                    scen_location.z = other_location.z
                    # print('actor location: ', scen_location, other_actors.type_id)
                    if calculate_distance(scen_location, other_location) < 2:
                        new_status = py_trees.common.Status.SUCCESS
                        break
        self.logger.debug("%s.update()[%s->%s]" % (self.__class__.__name__, self.status, new_status))
...
class Rectify(AtomicCondition):
    ...
    def __init__(self, status_list, message, name):
        """
        Setup actor
        """
        super(Rectify, self).__init__(name)
    ...
    def initialise(self):
        ...
        Initialize the start time of this condition
        """
        super(Rectify, self).initialise()

    def update(self):
        """
        Check if the _actor stands still (v=0)
        """
        self.status_list[0] = self.message
        new_status = py_trees.common.Status.SUCCESS
        return new_status


class HasBeenOccupied(AtomicCondition):
    """
    This class contains a standstill behavior of a scenario
    ...
    def __init__(self, ego_vehicle, actor, name, duration=float("inf")):
        ...
        Setup actor
        """
        super(HasBeenOccupied, self).__init__(name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._actor = actor
        self.ego_vehicle_id = ego_vehicle.id
    ...
    def initialise(self):
        ...
        Initialize the start time of this condition
        """
        super(HasBeenOccupied, self).initialise()
    ...
    def update(self):
        ...
        #     return new_status
        for _, other_actors in CarlaDataProvider.get_actors():
            if other_actors.id != self._actor.id and other_actors.id != self.ego_vehicle_id:
                other_location = CarlaDataProvider.get_location(other_actors)
                if calculate_distance(scen_location, other_location) < 5:
                    new_status = py_trees.common.Status.SUCCESS
                    break
        self.logger.debug("%s.update()[%s->%s]" % (self.__class__.__name__, self.status, new_status))
        self._has_checked = True
...
class TooFarAway(AtomicCondition):
    """
    This class contains a standstill behavior of a scenario
    ...
    def __init__(self, ego_vehicle, actor, name, duration=float("inf")):
        ...
        Setup actor
        """
        super(TooFarAway, self).__init__(name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._actor = actor
    ...
    def initialise(self):
        ...
        Initialize the start time of this condition
        """
        super(TooFarAway, self).initialise()
    ...
    def update(self):
        ...
        other_location = CarlaDataProvider.get_location(self.ego_vehicle)
        ego_vehicle_z_value = other_location.z
        other_location.z = 0
        if calculate_distance(scen_location, other_location) > 20 or ego_vehicle_z_value >= 0.2:
            print('too far away!')
            new_status = py_trees.common.Status.SUCCESS
            self._has_checked = True
...
class DriveDistance(AtomicCondition):
    ...
    def update(self):
        ...
        self._location.z = new_location.z
