...
class DrivenDistanceTest(Criterion):
    ...
    def terminate(self, new_status):
        ...
        print('terminate in DrivenDistanceTest')
...
class CollisionTest(Criterion):
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            print('in CollisionTest, new status is ', new_status)
            new_status = py_trees.common.Status.FAILURE
        ...
        if self.other_actor_type:
            if self.other_actor_type == "miscellaneous":
                ...
                if self.other_actor_type not in event.other_actor.type_id:
                    print('self.other_actor_type not in event.other_actor.type_id:')
                    return
...
class ActorSpeedAboveThresholdTest(Criterion):
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            print('failure in ActorSpeedAboveThresholdTest')
            new_status = py_trees.common.Status.FAILURE
        self.logger.debug("%s.update()[%s->%s]" % (self.__class__.__name__, self.status, new_status))
...
class OnSidewalkTest(Criterion):
    ...
    def terminate(self, new_status):
        ...
        if self._outside_lane_active:
            ...
            self._wrong_outside_lane_distance = 0
            self.list_traffic_events.append(outsidelane_event)
        print('terimante in OnSidewalkTest')
...
class OutsideRouteLanesTest(Criterion):
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            print('new status is FAILURE in OutsideRouteLanesTest')
            new_status = py_trees.common.Status.FAILURE

        # Some of the vehicle parameters
        location = CarlaDataProvider.get_location(self._actor)
        if location is None:
            print('in OutsideRouteLanesTest, new_status is ', new_status)
            return new_status
...
#                 #     #     off_route = True
#                 #     if self._out_route_distance > self.MAX_ROUTE_PERCENTAGE:
#                 #         off_route = True
#
...
class RouteCompletionTest(Criterion):
    ...
    def __init__(self, actor, route, name="RouteCompletionTest", terminate_on_failure=False):
        ...
        self._route_length = len(self._route)
        self._waypoints, _ = zip(*self._route)
        # print('in atomic_criteria: ', len(self._waypoints), self._waypoints)
...
class RunningRedLightTest(Criterion):
    ...
    # todo: change
    DISTANCE_LIGHT = 10  # m
    APPROACH_LIGHT = 1.5 * DISTANCE_LIGHT
    ...
    def __init__(self, actor, name="RunningRedLightTest", terminate_on_failure=False):
        ...
        for _actor in all_actors:
            if 'traffic_light' in _actor.type_id:
                # todo: change red, green and yellow time
                # print('in automic_creteria')
                _actor.set_green_time(5)
                _actor.set_red_time(0.5)
                _actor.set_yellow_time(3)
    ...
    def update(self):
        ...
        for traffic_light, center, waypoints in self._list_traffic_lights:
            ...
            # todo: add the approach light event
            for wp in waypoints:
                ...
                if tail_wp.road_id == wp.road_id and tail_wp.lane_id == wp.lane_id and dot_ve_wp > 0:
                    ...
                    # todo: change to center waypoint
                    # location_wp = center_loc

                    lft_lane_wp = self.rotate_point(carla.Vector3D(0.4 * lane_width, 0.0, location_wp.z),
                                                    yaw_wp + 90)
                    lft_lane_wp = location_wp + carla.Location(lft_lane_wp)
                    rgt_lane_wp = self.rotate_point(carla.Vector3D(0.4 * lane_width, 0.0, location_wp.z),
                                                    yaw_wp - 90)
                    ...
                    if self.is_vehicle_crossing_line((tail_close_pt, tail_far_pt), (lft_lane_wp, rgt_lane_wp)):
                        self._last_light_id = traffic_light.id
                        break

                    back_location = self.rotate_point(carla.Vector3D(-2 * veh_extent, 0.0, location.z),
                                                      transform.rotation.yaw)
                    back_location = location + carla.Location(back_location)
                    if center_loc.distance(
                            location) < self.APPROACH_LIGHT and traffic_light.id != self._last_light_id:
                        approach_light_event = TrafficEvent(event_type=TrafficEventType.APPROACH_LIGHT)
                        approach_light_event.set_dict(
                            {'distance': center_loc.distance(back_location),
                             'id': traffic_light.id,
                             'state': traffic_light.state,
                             'lft_lane_wp': lft_lane_wp,
                             'rgt_lane_wp': rgt_lane_wp,
                             'tail_close_pt': tail_close_pt,
                             'tail_far_pt': tail_far_pt})
                        self.list_traffic_events.append(approach_light_event)
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            print('terminate in traffic light test')
            new_status = py_trees.common.Status.FAILURE
