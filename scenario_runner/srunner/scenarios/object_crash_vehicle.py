...
class DynamicObjectCrossing(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False,
                 debug_mode=False, criteria_enable=True, adversary_type=False, timeout=60, log_file=None):
        """
        Setup all relevant parameters and create scenario
        """
        self._wmap = CarlaDataProvider.get_map()
    ...
    def _spawn_adversary(self, transform, orientation_yaw):
        ...
        if self._adversary_type is False:
            ...
            first_vehicle = CarlaDataProvider.request_new_actor('vehicle.tesla.model3', transform)
    ...
    def _initialize_actors(self, config):
        ...
        self._disp_transform = disp_transform
        self._prop_disp_transform = prop_disp_transform
    ...
    def _create_behavior(self):
        ...
        real_lane_width = self._reference_waypoint.lane_width
        ...
        detect_collision = WalkerCollision(self.ego_vehicles[0], self.other_actors[0], name="detect collision")
        ...
        actor_drive = DriveDistance(self.other_actors[0],
                                    0.8 * real_lane_width + 1.25 * (self._num_lane_changes - 2) * real_lane_width,
                                    name="walker drive distance")
        # actor_drive = DriveDistance(self.other_actors[0],
        #                             0.5 * lane_width,
        #                             name="walker drive distance")
        # todo: add stand still
        stand_still = KeepVelocity(self.other_actors[0],
                                   0,
                                   name="walker standstill")
        duration_time = np.random.randint(1, 7, 1)[0]

        time_over = StandStill(self.other_actors[0], name="stand time over", duration=duration_time)
        ...
        stand_on_road = py_trees.composites.Parallel(
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE, name="stand on road")
        ...
        # todo: add
        detect_collision_condition = py_trees.composites.Sequence()
        detect_collision_condition.add_child(detect_collision)
        detect_collision_condition.add_child(Rectify(self.status, 'finished', name="object_crash_vehicle"))

        detect_collision_condition.add_child(actor_remove)
        detect_collision_condition.add_child(static_remove)
        root.add_child(detect_collision_condition)
        ...
        # scenario_sequence1 = py_trees.composites.Parallel(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        # scenario_sequence2 = py_trees.composites.Sequence()
        # scenario_sequence1.add_child(scenario_sequence2)
        # scenario_sequence1.add_child(TooFarAway(self.ego_vehicles[0], self.other_actors[1], name="TooFarAway"))


        scenario_sequence.add_child(start_condition)
        scenario_sequence.add_child(keep_velocity)
        scenario_sequence.add_child(Rectify(self.status, 'running', name="object_crash_vehicle"))
        scenario_sequence.add_child(stand_on_road)
        scenario_sequence.add_child(Rectify(self.status, "success", name='object_crash_vehicle'))
        ...
        stand_on_road.add_child(stand_still)
        stand_on_road.add_child(time_over)
...
#         self._spawn_attempted = 0
#         self._spawn_status = True
#         self._disp_transform = None
#         self._prop_disp_transform = None
#         self._ego_route = CarlaDataProvider.get_ego_vehicle_route()
#
#         super(DynamicObjectCrossing, self).__init__("DynamicObjectCrossing",
#                                                     ego_vehicles,
...
#         real_lane_width = self._reference_waypoint.lane_width
#         dist_to_trigger = 12 + self._num_lane_changes
#         print('in _create_behavior')
#         print('location:', self.transform.location)
#         # leaf nodes
#         if self._ego_route is not None:
#             start_condition = InTriggerDistanceToLocationAlongRoute(self.ego_vehicles[0],
#                                                                     self._ego_route,
...
#                                    0,
#                                    name="walker standstill")
#         time_over = StandStill(self.other_actors[0], name="stand time over", duration=6)
#
#         actor_start_cross_lane = AccelerateToVelocity(self.other_actors[0],
#                                                       1.0,
#                                                       self._other_actor_target_velocity,
...
#         # static_remove = ActorDestroy(self.other_actors[1],
#         #                              name="Destroying Prop")
#         actor_remove = ActorTransformSetter(self.other_actors[0], self._disp_transform, name='remove pedestrians',
#                                             physics=False)
#         static_remove = ActorTransformSetter(self.other_actors[1], self._prop_disp_transform,
#                                              name='remove static prop', physics=False)
#         # end_condition = DriveDistance(self.ego_vehicles[0],
#         #                               self._ego_vehicle_distance_driven,
#         #                               name="End condition ego drive distance")
#
...
#
#
#         scenario_sequence2.add_child(start_condition)
#         scenario_sequence2.add_child(keep_velocity)
#         scenario_sequence2.add_child(Rectify(self.status, 'running', name="object_crash_vehicle"))
#         scenario_sequence2.add_child(stand_on_road)
#         scenario_sequence2.add_child(Rectify(self.status, "success", name='object_crash_vehicle'))
#         scenario_sequence2.add_child(keep_velocity_other)
#         scenario_sequence2.add_child(actor_stop_crossed_lane)
#
#         scenario_sequence.add_child(scenario_sequence1)
#
#         scenario_sequence.add_child(actor_remove)
#         scenario_sequence.add_child(static_remove)
#         # scenario_sequence.add_child(end_condition)
